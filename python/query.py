...
def getPass():
	try:
		...
		print('[ERROR] unable to get dm password from enviorment variables!')
class stat:
	def __init__(self,name,mode='r',desc=None,staroll=None)
		self.mode = mode
		if 'c' in self.mode:
			#we are allowed to create stats 
		if 'w' in self.mode:
			#we are allowed to write to existing stats
		if 'r' in self.mode:
			#we are allowed to read from an existing stat
			if desc == None or staroll == None:
				conn = san.getconn()
				#we were not given a description, so read one in from the database
